...
- `stable` 只是可移动的晋升指针；部署建议锁定具体 release tag。
- Upstream Check 会把 ZeroTier release tag 解析为 commit SHA；已固定 tag 若发生 retag，会失败并要求人工检查。
- PLANET 与 Controller 共用一份 ZeroTier 二进制和版本固定值，但仍使用两个独立进程、identity 和数据目录。Upstream Check 只通过自动 PR 提议新的共享 ZeroTier 版本，必须通过 Controller API、ztncui CRUD、重启持久化和 Upgrade Test 后人工合并。
- Controller 1.16.2 使用 `ZT_NONFREE=1`；本项目限定个人、非商业研究用途。
