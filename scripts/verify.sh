...
echo "OK: Compose syntax and PLANET endpoint look valid."
echo "NOTE: expose UDP/TCP ${PLANET_ZT_PORT:-9994} for PLANET and UDP ${CONTROLLER_ZT_PORT:-9993} for Controller VL1 traffic."
echo "NOTE: Controller HTTP API is internal-only; ztncui talks to 127.0.0.1 inside the business container."
echo "NOTE: ztncui UI and PLANET file service are loopback-only by default on the host."
